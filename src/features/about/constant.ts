import { Steps } from "./types";

export const OUR_QUALITIES: Steps[] = [
  {
    id: 1,
    title: `Creativity`,
    description:
      "We push boundaries with fresh, innovative ideas that bring your brand to life and capture your audience's attention",
    icon: "maginifier_briefcase",
  },
  {
    id: 2,
    title: `Teamwork`,
    description:
      "Next, we either create or refine your brand identity, aligning it with your business vision and ensuring it resonates with your audience.",
    icon: "paintbrush",
  },
  {
    id: 3,
    title: `Simplicity`,
    description:
      "We develop a tailored digital marketing plan, including SEO, PPC, content creation, and branding to drive growth and meet your specific objectives.",
    icon: "blueprint_bulb",
  },
  {
    id: 4,
    title: `Quality`,
    description:
      "Finally, we launch your campaigns, continuously monitoring and optimizing them for improved performance, higher lead generation, and maximum ROI.",
    icon: "bar_chart_up",
  },
];

export const BRANDS = [
  {
    logo: "/logos/aura.svg",
    title: "Aura Group",
  },
  {
    logo: "/logos/bumbyy.png",
    title: "Bumbby",
  },
  {
    logo: "/logos/famous-daves.png",
    title: "Famous Daves",
  },
  {
    logo: "/logos/kf.png",
    title: "Simply kf",
  },
  {
    logo: "/logos/phenom.png",
    title: "Phenom",
  },
  {
    logo: "/logos/saj-legal.png",
    title: "Saj Legals",
  },
  {
    logo: "/logos/mc-logo.svg",
    title: "MC Bauchemie",
  },
  {
    logo: "/logos/mine-hive.png",
    title: "Mine Hive",
  },
];
